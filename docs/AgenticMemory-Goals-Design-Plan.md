...
- M9: Shipped the MCP `summarize` tool plus summarization pipeline, adding provenance (`source_memory_ids`), idempotent `summary_key` tagging, Ollama-backed abstractive summaries with deterministic extractive fallback, new configuration knobs (`SUMMARIZATION_PROVIDER`, `SUMMARIZATION_MODEL`, `SUMMARIZATION_MAX_WORDS`), and documentation/tests proving discoverability via `search`.
